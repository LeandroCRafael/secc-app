...
describe("cliente CVM", () => {
  ...
  it("mapeia contas e derivados da DFP para R$ milhões", async () => {
    ...
    zip.file(
      ...
      header + row("1", "300000") + row("1.01.01", "10000") + row("1.01.02", "5000")
    );
    zip.file(
      "dfp_cia_aberta_BPP_con_2025.csv",
      header + row("2.03", "80000") + row("2.01", "90000")
    );
    zip.file(
      "dfp_cia_aberta_DFC_MD_con_2025.csv",
      header + row("6.01", "20000") + row("6.05", "3000")
    );
    zip.file("dfp_cia_aberta_DVA_con_2025.csv", header + row("7.08.01", "15000"));
    const buffer = await zip.generateAsync({ type: "uint8array" });
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(new Response(new Uint8Array(buffer).buffer as ArrayBuffer))
    );
  ...
  it("preserva o ponto decimal usado pela CVM antes de converter a escala", async () => {
    const zip = new JSZip();
    zip.file("dfp_cia_aberta_DRE_con_2025.csv", header + row("3.01", "1416020.0000000000"));
    zip.file("dfp_cia_aberta_BPA_con_2025.csv", header + row("1", "8837570.0000000000"));
    zip.file("dfp_cia_aberta_BPP_con_2025.csv", header + row("2.03", "-11748150.0000000000"));
    const buffer = await zip.generateAsync({ type: "uint8array" });
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(new Response(new Uint8Array(buffer).buffer as ArrayBuffer))
    ...
    const result = await collectCvmDfp("12.345.678/0001-99", 2025);
    const revenue = result.metrics.find((metric) => metric.variable === "Receita Líquida");
    expect(revenue?.value).toBe(1416.02);
  });
});
