...
export async function searchCvmCompanies(query: string): Promise<CvmCompany[]> {
  ...
  return rows
    .map((row) => ({
      ...
      corporateName: row.DENOM_SOCIAL ?? "",
      tradeName: row.DENOM_COMERC ?? "",
      cvmCode: row.CD_CVM ?? "",
      sector: row.SETOR_ATIV ?? "",
      status: row.SIT ?? row.SIT_EMISSOR ?? ""
    }))
    .map((company) => {
    ...
    .sort(
      (a, b) => b.score - a.score || a.company.corporateName.localeCompare(b.company.corporateName)
    )
    .slice(0, 20)
    .map(({ company }) => company);
}
...
function parseCvmNumber(row: CvmRow): number | null {
  const source = (row.VL_CONTA ?? "").trim();
...
function latestRows(rows: CvmRow[], cnpj: string): CvmRow[] {
  const selected = rows.filter(
    (row) =>
      digits(row.CNPJ_CIA ?? "") === cnpj &&
      normalizeCompanyName(row.ORDEM_EXERC ?? "ultimo").includes("ultimo")
  );
...
function add(
  ...
  if (value !== null && Number.isFinite(value))
    metrics.push({ variable, value, accountCode, statement, basis });
}
...
function findFile(
  ...
  return Object.values(zip.files).find(
    (file) =>
      !file.dir &&
      file.name.toLowerCase().includes(fragment.toLowerCase()) &&
      file.name.toLowerCase().includes(marker)
  );
}
...
async function statementRows(
  ...
  fragment: string,
  cnpj: string
): Promise<{ rows: CvmRow[]; basis: string }> {
  ...
  return {
    rows: latestRows(csvRecords(decode(await file.async("uint8array"))), cnpj),
    basis: consolidated ? "consolidado" : "individual"
  };
}

export async function collectCvmDfp(
  cnpjInput: string,
  year: number
): Promise<{ metrics: CvmMetric[]; sourceUrl: string; basis: string }> {
  ...
  const [dre, bpa, bpp, dfc, dva] = await Promise.all([
    statementRows(zip, "_DRE_", cnpj),
    statementRows(zip, "_BPA_", cnpj),
    statementRows(zip, "_BPP_", cnpj),
    statementRows(zip, "_DFC_", cnpj),
    statementRows(zip, "_DVA_", cnpj)
  ]);
  if (dre.rows.length === 0 && bpa.rows.length === 0)
    throw new Error("A empresa não foi localizada na DFP selecionada.");
  ...
  add(
    metrics,
    "CMV",
    revenue !== null && grossProfit !== null ? revenue - grossProfit : null,
    "3.01 - 3.03",
    "DRE",
    `${dre.basis}; derivado`
  );
  ...
  add(
    ...
    ebit !== null && grossProfit !== null ? ebit - grossProfit : null,
    "3.05 - 3.03",
    "DRE",
    `${dre.basis}; derivado`
  );
  ...
  add(
    metrics,
    "Lucro Líquido",
    account(dre.rows, "3.11") ?? account(dre.rows, "3.11.01"),
    "3.11",
    "DRE",
    dre.basis
  );
