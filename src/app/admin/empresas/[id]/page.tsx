...
export default async function CompanyResearchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Search> }) {
  ...
  return <>
    ...
    <section className="section">
      <div className="panel-heading"><div><p className="eyebrow">Histórico</p><h2>Propostas da empresa</h2></div><div className="actions"><Link className="button secondary" href={`/admin/importacoes?companyId=${encodeURIComponent(company.id)}`}>Validar arquivo</Link><Link className="button secondary" href="/admin/revisoes">Abrir fila de revisão</Link></div></div>
